  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">宝宝月龄/阶段</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">照护焦点</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">科学育儿与产后护理避坑指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 科学育儿与母婴照护专家');
...
const metricsList = [
  {
    "key": "scientificPediatricRigor",
    "label": "科学育儿严谨度"
  },
  {
    "key": "infantNutritionBalance",
    "label": "婴幼儿膳食营养分"
  },
  {
    "key": "developmentalMilestoneMatch",
    "label": "发育里程碑契合度"
  },
  {
    "key": "postpartumCareComfort",
    "label": "产后护理解焦虑度"
  },
  {
    "key": "practicalOperability",
    "label": "实操指南易用性"
  }
];
const guideRules = [
  {
    "title": "辅食添加渐进原则",
    "advice": "辅食添加遵循“由少到多、由稀到稠、由单一到混合”，每引入一种新食物观察 3 天",
    "avoid": "严禁 1 岁以内宝宝食物中添加食盐、蜂蜜或过重调味品"
  },
  {
    "title": "肠胀气科学缓解",
    "advice": "喂奶后勤拍嗝，顺时针按摩宝宝腹部，适度进行飞机抱与脚踏车排气操",
    "avoid": "切忌宝宝一哭闹就频繁喂奶导致过度喂养加重肠胃负担"
  },
  {
    "title": "尊重个体发育差异",
    "advice": "大动作发育（翻身、坐、爬、走）存在 1-2 个月个体差异，多提供安全地面练习机会",
    "avoid": "避免过早使用学步车强行拉扯宝宝站立"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[MUYING_SCORES\][\s\S]*?\[\/MUYING_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[MUYING_SCORES\](.*?)\[\/MUYING_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('muying_history_records');
...
const saveHistory = () => {
  localStorage.setItem('muying_history_records', JSON.stringify(historyList.value));
};
